...
$(kernel_object_files): build/kernel/%.o : src/kernel/%.c
	mkdir -p $(dir $@) && \
	./toolchain/bin/x86_64-elf-gcc -c -Isrc/ -ffreestanding $(patsubst build/kernel/%.o, src/kernel/%.c, $@) -o $@ -g -DDEBUG

$(lib_object_files): build/lib/%.o : src/lib/%.c
	mkdir -p $(dir $@) && \
		./toolchain/bin/x86_64-elf-gcc -c -Isrc/ -ffreestanding $(patsubst build/lib/%.o, src/lib/%.c, $@) -o $@ -g -DDEBUG
